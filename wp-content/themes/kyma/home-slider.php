	...
	<?php if(isset($kyma_theme_options['home_slider_posts']) && $kyma_theme_options['home_slider_posts'] != "") {
        ...
        foreach ($kyma_theme_options['home_slider_posts'] as $post_id) {
			...
			<span class="owl_text_a">
                ...
                <span class="owl_text_b"><span
                        id="slide-subtitle-<?php echo $i; ?>"><?php echo esc_attr(wp_trim_words($slider->post_content, 8, '...')); ?></span></span>
                <span class="owl_text_c"><span>
                <a id="slide-description-<?php echo $i; ?>"  href="<?php echo esc_url(get_post_permalink($slider->ID)); ?>" target="_self"><?php _e('Read More', 'kyma'); ?></a></span></span>
            </div>
            </div><?php
            $i++;
        } ?>
